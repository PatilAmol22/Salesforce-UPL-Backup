...
import GRZ_Brazil_budget_Acess_Users from '@salesforce/label/c.GRZ_Brazil_budget_Acess_Users';
...
export default class DemandGeneration extends NavigationMixin(LightningElement) {
  ...
  @track profName=''; // add this for RITM0556384 GRZ(Javed Ahmed) 25-05-2023
...
*/
  ...
  userDetails3({ error, data }) {
    if (data) {
      this.profName=data.fields.Profile.displayValue;// add this for RITM0556384 GRZ(Javed Ahmed) 25-05-2023
  ...
  userDetails1({ error, data }) {
    ...
    //  Update below Condition for INC0472766 GRZ(Dheeraj Sharma) 05-05-2023
    var budgetUser = GRZ_Brazil_budget_Acess_Users;
    var bdgtUser =budgetUser.split(",");
    var profileName =this.profName; //add this for RITM0556384 GRZ(Javed Ahmed) 25-05-2023
    console.log('budgetUser', bdgtUser ,'profileName',profileName);

    if (data) {
      console.log('User data is', data);
      console.log('User name in New ', data.fields.Name.value);
      for (var i = 0; i < bdgtUser.length; i++) {
        if (data.fields.Name.value == bdgtUser[i] || profileName ==bdgtUser[i]) { //add profileName chcek for RITM0556384 GRZ(Javed Ahmed) 25-05-2023
          this.profileName1 = true;
          console.log('bdgtUser[i]', bdgtUser[i]);
        }
      }
 ...
 */
  ...
  updatevalue(event) {
    ...
    if (field == 'Credit Letter') {
     // this.demand.creditLetter = value;   // Updated  Condition for RITM0551279 GRZ(Dheeraj Sharma) 10-05-2023
    }
    if (field == 'Payment to Supplier') {
    // this.demand.paymenttoSupplier = value;  // Updated  Condition for RITM0551279 GRZ(Dheeraj Sharma) 10-05-2023
    }
    ...
    if (field == 'Payment Method') {
      ...
      // Added below Condition for RITM0551279 GRZ(Dheeraj Sharma) 10-05-2023
      if(this.demand.paymentMethod=='Payment to Supplier'){
        this.demand.paymenttoSupplier='Yes';
        this.demand.creditLetter='';
      }
      if(this.demand.paymentMethod=='Credit Letter'){
          this.demand.creditLetter='Yes';
          this.demand.paymenttoSupplier='';
        }
